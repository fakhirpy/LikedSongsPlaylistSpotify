...
client_id = 'https://developer.spotify.com/dashboard'
...
playlist = sp.user_playlist_create(user=sp.current_user(
)['id'], name=playlist_name, public=True, description=playlist_description)
...
while True:
    ...
    for item in results['items']:
        ...
        tracks.append(track)
    sp.user_playlist_add_tracks(user=sp.current_user(
    )['id'], playlist_id=playlist['id'], tracks=tracks)
    offset += limit
    if len(tracks) < limit:
        break
